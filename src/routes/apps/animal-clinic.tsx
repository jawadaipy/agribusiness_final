...
const AnimalClinicPage = () => {
  ...
  const [tagsInput, setTagsInput] = useState("Cattle, Dairy, Fever");
  ...
  const loadCases = async () => {
    ...
    if (postsResult.error) {
      setLoadError("Livestock clinical cases could not be loaded from database.");
      setCases([]);
      setIsLoading(false);
      return;
    }
    ...
    if (posts.length > 0) {
      ...
      const commentsByPost = new Map<string, AnimalComment[]>();
      for (const c of (commentsResult.data ?? []) as AnimalComment[]) {
        const author = commentProfileMap.get(c.profile_id);
        const enrichedComment: AnimalComment = {
          ...c,
          authorName: author?.display_name || "Doctor of Veterinary Medicine",
          authorRole: author?.user_type ? author.user_type.toUpperCase() : "VETERINARIAN",
        };
      ...
      const enrichedPosts: AnimalCase[] = posts.map((p) => {
        const owner = profileMap.get(p.profile_id);
        return {
          ...p,
          posterName: owner?.display_name || "Livestock Producer",
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    ...
    if (title.trim().length < 5 || body.trim().length < 10) {
      setSubmitError("Provide a detailed condition description (at least 10 characters).");
      return;
    }
    ...
    if (error) {
      ...
      setSubmitSuccess("Your livestock case has been submitted for certified veterinary review.");
      setTitle("");
      setBody("");
      setImageUrl("");
      await loadCases();
      ...
      <main className="pb-16 pt-24">
        <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
          ...
          <div className="mx-auto mb-8 max-w-5xl text-left">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-amber-700 via-orange-800 to-amber-950 text-white shadow-xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-20 -mt-20 pointer-events-none" />
              <div className="relative z-10 space-y-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-[10px] font-bold uppercase tracking-wider text-amber-200">
                  <span className="material-symbols-outlined text-[14px]">pets</span>
                  24/7 Livestock & Dairy Telehealth
                </div>
                ...
                <p className="max-w-xl text-xs sm:text-sm font-medium leading-relaxed text-white/85">
                  Direct clinical advisory for cattle, buffalo, poultry, and small ruminants with certified veterinarians from Partner University Animal Sciences.
                </p>
              </div>
              ...
              <div className="relative z-10 flex sm:flex-col items-center sm:items-end gap-2 shrink-0">
                <div className="px-4 py-2 rounded-2xl bg-white/10 border border-white/15 backdrop-blur-sm text-center">
                  <span className="text-[10px] uppercase font-bold text-white/70 block">Certified Vets</span>
                  <span className="font-display text-2xl font-black text-amber-300">850+</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-12">
            ...
            <div className="space-y-6 lg:col-span-8 text-left">
              ...
              <form onSubmit={handleReportSubmit} className="rounded-3xl border border-outline-variant/40 bg-white p-6 sm:p-7 card-shadow">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-800">
                    <span className="material-symbols-outlined text-[20px] text-amber-600">add_circle</span>
                    Report Livestock Symptoms / Emergency
                  </h3>
                  <span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-wider">Telehealth Consult</span>
                </div>
                ...
                <div className="space-y-3">
                  <input
                    ...
                    placeholder="Case Headline (e.g. Sahiwal Cow showing high fever and reduced milk output)"
                    className="w-full rounded-xl border border-outline-variant/50 bg-surface-container-low px-4 py-3 text-xs font-medium text-primary outline-none focus:border-amber-600 transition-all"
                  />
                  ...
                  <textarea
                    ...
                    placeholder="Describe symptoms, temperature, herd count, feeding regime, vaccinations, and duration of illness..."
                    className="w-full resize-none rounded-xl border border-outline-variant/50 bg-surface-container-low px-4 py-3 text-xs font-medium text-primary outline-none focus:border-amber-600 transition-all"
                  />
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <input
                      ...
                      placeholder="Species & tags (e.g. Dairy Cattle, Mastitis, Okara)"
                      className="w-full rounded-xl border border-outline-variant/50 bg-surface-container-low px-4 py-2.5 text-xs font-medium text-primary outline-none focus:border-amber-600 transition-all"
                    />
                    <input
                      value={imageUrl}
                ...
                <div className="mt-4 pt-4 border-t border-outline-variant/30 flex items-center justify-between">
                  <span className="text-[10px] text-on-surface-variant/60 font-medium">
                    Emergency cases prioritized by DVMs
                  </span>
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="px-6 py-3 bg-gradient-to-r from-amber-600 to-orange-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider hover:opacity-90 shadow-md transition-all flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
                  >
                    {isSubmitting ? "Consulting..." : "Consult Veterinarian"}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                  <h2 className="text-xs font-bold uppercase tracking-wider text-amber-900 flex items-center gap-2">
                    <span className="material-symbols-outlined text-[18px] text-amber-600">health_and_safety</span>
                    Active Veterinary Cases ({cases.length})
                  </h2>
                </div>
                ...
                {isLoading ? (
                  ...
                  <div className="rounded-3xl border border-dashed border-outline-variant/70 bg-white p-12 text-center card-shadow">
                    <div className="w-14 h-14 rounded-2xl bg-amber-50 flex items-center justify-center mx-auto mb-3">
                      <span className="material-symbols-outlined text-3xl text-amber-700/50">pets</span>
                    </div>
                    <h3 className="font-display text-lg font-bold text-amber-900">No Veterinary Cases Reported Yet</h3>
                    <p className="mt-1 text-xs text-on-surface-variant font-medium max-w-sm mx-auto">
                      Submit animal health symptoms above to receive immediate guidance from certified DVM practitioners.
                    </p>
                  </div>
                ) : (
                  cases.map((problem) => (
                    ...
                    >
                      <div className="mb-4 flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-amber-600 to-orange-700 text-white font-bold text-xs shadow-sm">
                            {problem.posterName?.charAt(0) || "L"}
                          </div>
                          <div>
                        ...
                        {problem.is_resolved ? (
                          ...
                          <span className="badge-gold">In Consultation</span>
                        )}
                      </div>
                      ...
                      {problem.comments && problem.comments.length > 0 && (
                        <div className="space-y-3 border-l-2 border-amber-500/30 pl-4 my-4">
                          {problem.comments.map((comment) => (
                            <div key={comment.id} className="flex gap-3">
                              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-amber-600 text-white shadow-sm mt-1">
                                <span className="material-symbols-outlined text-[16px]">medical_services</span>
                              </div>
                              <div className="flex-1 rounded-2xl rounded-tl-none border border-outline-variant/30 bg-amber-50/40 p-4">
                                <div className="mb-1 flex items-center gap-2">
                                  <span className="text-xs font-bold text-amber-950">{comment.authorName}</span>
                      ...
                      <div className="mt-4 flex gap-2 pt-3 border-t border-outline-variant/20">
                        <input
                          ...
                          placeholder="Provide veterinary advice or clinical diagnosis..."
                          className="flex-1 rounded-xl border border-outline-variant/40 bg-surface-container-low px-3.5 py-2.5 text-xs text-primary outline-none focus:border-amber-600 font-medium"
                        />
                        <button
                          type="button"
                          disabled={isReplying === problem.id}
                          onClick={() => void handleAddComment(problem.id)}
                          className="px-5 py-2.5 bg-gradient-to-r from-amber-600 to-orange-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider hover:opacity-90 disabled:opacity-50 cursor-pointer shrink-0"
                        >
                          Advise
                        </button>
                      </div>
            ...
            <aside className="space-y-6 lg:col-span-4 text-left">
              <div className="rounded-3xl bg-gradient-to-br from-amber-700 via-orange-800 to-amber-950 p-6 text-white card-shadow relative overflow-hidden">
                ...
                <h4 className="relative z-10 mb-4 font-display text-base font-bold tracking-tight text-white flex items-center gap-2">
                  <span className="material-symbols-outlined text-amber-300 text-[20px]">vaccines</span>
                  Veterinary Advisory Protocol
                </h4>
                <div className="relative z-10 space-y-3 text-xs">
                  <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
                    <span className="font-bold text-white/70">Verified DVMs</span>
                    <span className="font-display font-black text-amber-200">850+</span>
                  </div>
                  <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
                    <span className="font-bold text-white/70">Species Covered</span>
                    <span className="font-bold text-amber-200">Cattle, Buffalo, Poultry</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-white/70">Emergency Hotline</span>
                    <span className="font-bold text-emerald-300">24/7 Available</span>
                  </div>
                </div>
              </div>

              <div className="rounded-3xl border border-outline-variant/40 bg-white p-6 card-shadow">
                <h4 className="mb-3 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-amber-900">
                  <span className="material-symbols-outlined text-[18px] text-amber-600">verified</span>
                  University Partnered Telehealth
                </h4>
                <p className="text-xs leading-relaxed text-on-surface-variant font-medium">
                  Direct partnership with University of Veterinary and Animal Sciences (UVAS) and Faisalabad Agriculture University alumni.
                </p>
                <Link
                  to="/search"
                  className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-amber-800 hover:text-amber-600 transition-colors"
                >
                  Find a Local Livestock Specialist
...
export const Route = createFileRoute("/apps/animal-clinic")({
  head: () => ({
    title: "Animal & Vet Clinic | AgriBusiness Pakistan",
    meta: [

      ...
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-6xl overflow-hidden rounded-3xl border border-outline-variant/60 bg-white shadow-[0_18px_48px_rgba(15,81,50,0.12)] lg:grid-cols-[0.95fr_1.05fr]">
        <section className="relative overflow-hidden bg-primary p-8 text-on-primary sm:p-12">
          <div className="relative z-10">
            ...
            <p className="mt-16 text-[10px] font-bold uppercase tracking-[0.18em] text-secondary-container">Restricted governance access</p>
        ...
        <section className="flex items-center bg-surface-container-low p-6 sm:p-10 lg:p-14">
          <div className="w-full max-w-md">
            ...
            <p className="mt-10 text-[10px] font-bold uppercase tracking-[0.16em] text-secondary">Secure access</p>
            ...
            {checking ? <div className="mt-8 rounded-xl border border-outline-variant bg-white p-4 text-xs font-medium text-on-surface-variant">Checking existing secure session…</div> : <form onSubmit={handleSubmit} className="mt-8 space-y-5"><label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">Administrator email</span><input type="email" autoComplete="username" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" className="w-full rounded-xl px-4 py-3 text-sm" /></label><label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">Password</span><input type="password" autoComplete="current-password" required value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" className="w-full rounded-xl px-4 py-3 text-sm" /></label><button type="submit" disabled={loading} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3.5 text-xs font-bold uppercase tracking-[0.12em] text-on-primary transition hover:bg-primary-container disabled:opacity-50"><span className="material-symbols-outlined text-[17px]">lock</span>{loading ? "Verifying access…" : "Enter Super Admin portal"}</button></form>}

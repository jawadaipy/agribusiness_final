  ...
  return (
    <section className="rounded-2xl border border-outline-variant/60 bg-white p-5 shadow-[0_10px_28px_rgba(15,81,50,0.06)] md:p-7">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.14em] text-on-surface-variant/65">{isConsultant ? "Lead radar" : "Opportunity radar"}</p>
        ...
        <Link to="/projects" className="inline-flex w-fit items-center gap-1.5 rounded-xl border border-outline-variant/70 px-3.5 py-2.5 text-xs font-bold text-primary transition hover:bg-surface-container-low">
            ...
            <Link key={match.id} to="/projects/$id" params={{ id: match.id }} className="group block rounded-xl border border-outline-variant/50 bg-surface-container-low/60 p-4 transition hover:border-primary/40 hover:bg-white">
              <div className="flex flex-col justify-between gap-2 sm:flex-row">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-on-surface-variant">{match.description}</p>
                </div>
                <span className="h-fit shrink-0 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">{budget(match)}</span>
              </div>
              <div className="mt-2.5 flex flex-wrap gap-1.5">
                {match.hits.slice(0, 4).map((hit) => (
                  <span key={hit} className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-bold text-primary">matched: {hit}</span>
                ))}

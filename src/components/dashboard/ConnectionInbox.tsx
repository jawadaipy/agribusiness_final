...
export function ConnectionInbox({ profileId }: { profileId: string }) {
  ...
  return <section className="rounded-2xl border border-outline-variant/45 bg-surface-container-low p-5 md:p-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-on-surface-variant/65">Private relationship workflow</p><h2 className="mt-1 font-display text-2xl text-primary">Connections</h2><p className="mt-2 max-w-2xl text-xs leading-5 text-on-surface-variant">Requests are visible only to the sender and recipient. Contact details remain private until members decide how to continue.</p></div><button type="button" onClick={() => void loadRequests()} className="inline-flex items-center gap-1.5 text-xs font-bold text-primary"><span className="material-symbols-outlined text-[16px]">refresh</span>Refresh</button></div>{error ? <p className="mt-5 rounded-xl border border-error/25 bg-error/10 p-3 text-xs leading-5 text-error">{error}</p> : null}{notice ? <p className="mt-5 rounded-xl border border-primary/20 bg-primary/10 p-3 text-xs leading-5 text-primary">{notice}</p> : null}<div className="mt-6 grid gap-6 xl:grid-cols-2"><div><div className="mb-3 flex items-center justify-between"><h3 className="font-bold text-primary">Incoming requests</h3><span className="rounded-full bg-secondary-container px-2.5 py-1 text-xs font-bold text-on-secondary-container">{pendingIncoming.length} pending</span></div><div className="space-y-3">{incoming.length ? incoming.map((request) => personCard(request, request.requester_profile_id, "incoming")) : <EmptyRequestState label="No incoming requests" text="When another member asks to connect, it will appear here for your decision." />}</div></div><div><div className="mb-3 flex items-center justify-between"><h3 className="font-bold text-primary">Sent requests</h3><span className="rounded-full bg-surface-container-high px-2.5 py-1 text-xs font-bold text-on-surface-variant">{outgoing.length} total</span></div><div className="space-y-3">{outgoing.length ? outgoing.map((request) => personCard(request, request.recipient_profile_id, "outgoing")) : <EmptyRequestState label="No sent requests" text="Discover relevant people and send a connection request from their public profile." />}</div></div></div></section>;
}

  ...
  return (
    <section className="rounded-2xl border border-outline-variant/60 bg-white p-5 shadow-[0_10px_28px_rgba(15,81,50,0.06)] md:p-7">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.14em] text-on-surface-variant/65">Farm intelligence · {MONTH_NAMES[month - 1]}</p>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3">
          ...
          {weather ? (
            <>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[.12em] text-secondary-container">{weather.city} · now</p>
              ...
              <div className="mt-4 grid grid-cols-4 gap-2 border-t border-white/15 pt-3">
                {weather.days.map((day) => (
                  <div key={day.date} className="text-center">
                    ...
                    <p className="text-xs text-white/60">{day.min}°</p>
                    <p className={`text-xs font-bold ${day.rainChance >= 60 ? "text-secondary" : "text-white/50"}`}>{day.rainChance}%</p>
                  </div>
                ))}
              </div>
            </>
            ...
            <div className="flex h-full flex-col items-center justify-center py-8 text-center">
              ...
              <p className="mt-1 text-xs leading-4 text-white/70">The forecast service could not be reached. Seasonal guidance below is still valid.</p>
            </div>
          ) : (
        ...
        <div className="rounded-2xl border border-outline-variant/50 bg-surface-container-low/60 p-5">
          <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-[.12em] text-on-surface-variant">
            <span className="material-symbols-outlined text-[15px] text-primary">calendar_month</span>
            Crop calendar · {MONTH_NAMES[month - 1]}
          </p>
          <div className="mt-3">
            <p className="text-xs font-bold text-primary">Sowing window</p>
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {snapshot.sowNow.length === 0 ? (
                <span className="text-xs text-on-surface-variant">No major sowings this month</span>
              ) : (
                snapshot.sowNow.map((crop) => (
                  <span
                    key={crop.id}
                    title={crop.tip}
                    className={`rounded-full px-2.5 py-1 text-xs font-bold ${mineIds.has(crop.id) ? "bg-primary text-on-primary" : "bg-primary/10 text-primary"}`}
                  >
          ...
          <div className="mt-3">
            <p className="text-xs font-bold text-primary">Harvest window</p>
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {snapshot.harvestNow.length === 0 ? (
                <span className="text-xs text-on-surface-variant">No major harvests this month</span>
              ) : (
                snapshot.harvestNow.map((crop) => (
                  <span
                    key={crop.id}
                    title={crop.tip}
                    className={`rounded-full px-2.5 py-1 text-xs font-bold ${mineIds.has(crop.id) ? "bg-secondary text-primary" : "bg-secondary/30 text-[#6B4E00]"}`}
                  >
                    {crop.en} <span className="opacity-75">{crop.ur}</span>
                  </span>
                ))
          ...
          {mine.length > 0 ? (
            <p className="mt-3 rounded-xl bg-secondary-container p-2.5 text-xs leading-4 text-on-secondary-container">
              <span className="font-bold">Your crops this month:</span> {mine.map((c) => c.en).join(", ")}. Tap a chip for the field tip.
            </p>
          ) : myCrops.length === 0 ? (
            <p className="mt-3 text-xs leading-4 text-on-surface-variant">Add your crops under My profile so this calendar highlights your season.</p>
          ) : null}
        </div>
        ...
        <div className="rounded-2xl border border-outline-variant/50 bg-surface-container-low/60 p-5">
          ...
          {ratesUnavailable ? (
            <p className="mt-3 text-xs leading-5 text-on-surface-variant">Live mandi rates are not available right now. The ticker on the homepage carries indicative rates.</p>
          ) : (
      ...
      <div className="mt-4 rounded-2xl border border-secondary/30 bg-secondary-container/40 p-4">
        <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-[.12em] text-on-secondary-container">
          <span className="material-symbols-outlined text-[15px]">tips_and_updates</span>
          This week's field advisories
        </p>
        <ul className="mt-2.5 space-y-1.5">
          {advisories.map((note) => (
            <li key={note} className="flex items-start gap-2 text-xs leading-5 text-on-surface-variant">
              <span className="material-symbols-outlined mt-0.5 text-[13px] text-primary">arrow_right</span>
              {note}
            </li>
          ))}
        </ul>
        <p className="mt-2 text-xs leading-4 text-on-surface-variant/60">
        ...
        >
          <span className="flex items-center gap-3">
            ...
            <span>
              ...
              <span className="mt-0.5 block text-xs leading-4 text-on-surface-variant">Kisan Card, subsidized loans, crop insurance, and land records — check what you qualify for.</span>
            </span>
          </span>
